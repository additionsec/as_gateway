...
public class TransformJSONTest extends TestCase
{
    public TransformJSONTest( String testName ) {
        super( testName );
    }
    ...
    public static Test suite() {
        return new TestSuite( TransformJSONTest.class );
    }
	...
	public void testBasic() throws Exception {
		_transform = new TransformJSON( _prop );

		_transform.reconfigure(_prop);
		long ts = System.currentTimeMillis() / 1000;
		_transform.nowTick(ts);
		assertNotNull( _transform.description() );
		assertNull( _transform.header() );
		assertTrue( _transform.isMultiLine() );
		assertTrue( "json".equals(_transform.extension()) );
